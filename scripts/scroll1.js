...
// 스크롤 이동 시 인식하는 이벤트 종류 (멈츨 땐 인식안함)
...
window.addEventListener('scroll',()=>{
    ...
    // 각 섹션(4) 제목 aside(4) 색상 active로 활성화 하기
    for(let s of section){
        // console.log(s); // for~of 가 순회하는 대상 테스트
        const sTop = s.getBoundingClientRect().top;
        // console.log(sTop); // 각 섹션 top 위치값 테스트
        
        // 화면 상단(0)기준
        // 섹션의 윗(top)부분이 화면 위에서 150px 지점 보다 같거나 더 위로 올라와있다면 조건문 실행
        if(sTop <= 150){
    ...
    //nav.textContent = window.scrollY;
