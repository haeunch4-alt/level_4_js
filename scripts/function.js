...
// v1. 사용자정의함수를 이벤트함수 내에서 호출한 방식 (2개이상반복일경우)
// printBtn.addEventListener('click', printFunc);

// v2. 이벤트 함수 내에서 직접 사용자함수를 만드는 방식 반복 X
// 이벤트의 객체가 정해진 상태로 하기 때문에 내부 내장 함수엔 함수명을 따로 작성안함
...
// 4. 함수의 매개변수
// 반복되는 함수내에 달라지는 매개값이 있을 때 사용
// 동화 : 헨젤과 그레텔이 집을 찾아오는 반복 방법
function story1(boy='헨젤', girl='그레텔', mapData){// 함수생성
    let person = [boy,  girl];
    let result =person[0] + '과 '+ person[1]+ '이 ' + mapData + ' 따라 집을 찾아갔습니다.'
...
//동화 2 story2 함수 생성, 매개변수 달라지는 데이터 기준으로 추가 생성
// 나무꾼은 금도끼를 잃어버렸습니다.
// 나무꾼은 쇠도끼를 잃어버렸습니다.
function story2(perN='나무꾼', tool){
    let person = perN;
    let result = perN +'은 '+ tool + '를 잃어버렸습니다.'
    return console.log(result);
}
story2() // 매개변수 아무것도 안적었을 때 모두 기본값
story2('나무꾼', '금도끼'); // 앞의 매개변수 정의 안되서 기본값, 뒤 문자만 매개변수 대입
story2('나무꾼', '쇠도끼'); // 앞 매개변수 문자대입, 뒤매개변수 안적어서 기본값 처리

// 카페 키오스크
// 아이스아메리카노 1잔 주문완료
// function 간편버전 => 이벤츠 내에서 주 사용
// ''+ 변수 + '' 간편버전 `뮨자${변수}` 템플릿문자열
...
// 아이스, 핫, 메뉴, 메뉴개수까지 선택가능한 함수 만들기
// 출력 예) 아이스 카페라떼 2잔 주문완료
function kiosk(tem,order, number='1'){
    const result =`${tem} ${order} ${number}잔 주문완료`;
    return console.log(result);
}
kiosk('ICE', '카페라떼', 2)
kiosk('HOT', '카페라떼', 1)
...
function japangi(drink, num='1'){
    ...
    return; // 리턴 뒤에 아무것도 안적으면 람수만 강제 종료
    return console.log('잔액이 부족합니다.'); // 리턴 뒤 콘솔실행하고 함수 종료
    console.log(order); // 리턴 아래 써있는 명령은 절대 실행 안됨
}
japangi('생수') // 함수 내부 리턴에서 출력 함수가 정해졌을 때 데이터만 전달방식
console.log(japangi('사이다'));
// 함수 내부 리턴에서 출력함수가 정해지지 않고 데이터만 밖에 나와있을 때 함수 밖에서 출력함수를 정하는 방식
