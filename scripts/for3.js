// for3.js
// for~in
// 두개 이상의 인덱스로 이루어진 배열, 객체 DOM 요소 순회하는 반복문
const flower = ['장미', '민들레', '수선화', '나팔꽃'];
// for(let 초기변수 in 변수에 대입할 대상){반복처리}
for(let f in flower){
    console.log(f); //인덱스 출력 0 1 2 3
    console.log(flower[f])// for~in 이 추출한 인덱스를 활용해서 배열값 출력
}
...
//로그인 오류 검증 반복문+ 조건문(버튼클릭시실행)
const loginBtn = document.querySelector('#login_btn');
const inFrm = document.querySelector('#inFrm'); // 폼태그(안쪽 검사 input들 답기 위한 부모 대상)

// 로그인 버튼 클릭 시 수행 함수
loginBtn.addEventListener('click',()=>{
    const valFrm = {
        id:inFrm.user_id.value, // 아이디 인풋 값
        pw:inFrm.user_pw.value, // 속성:폼.비밀번호input.값
    }
    console.log(valFrm);// 위 변수 확인
...
// 특정 객체의 index 와 object 모두 사용할 땐 forEach
...
function resetFunc(target){
    for(let re of target){re.classList.remove('active')}
    return
}