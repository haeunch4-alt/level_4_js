...
// 할인률 계산기 (사용자 입력가격은 다르고, 할인은 고정)
// 계산법 ) (100-5) = 0.95
// 판매가 * 0.95 = 할인판매가
// *0.95 = 5%할인 
// *0.9 = 10%할인 
// *0.8 = 20%할인 
function discountFunc(userN){
    let userWrite = userN;
    let total5 = (userWrite * 0.95).toLocaleString('ko-kr');
    let total10= (userWrite * 0.9).toLocaleString('ko-kr');
    let total20 = (userWrite * 0.8).toLocaleString('ko-kr');
    let total30 = (userWrite * 0.7).toLocaleString('ko-kr');
    let total40 = (userWrite * 0.6).toLocaleString('ko-kr');
    let total50 = (userWrite * 0.5).toLocaleString('ko-kr');
    return `사용자 입력가격: ${userN.toLocaleString('ko-kr')}원 기준
    5% 할인가 :${total5}원,
    10% 할인가 :${total10}원,
    20% 할인가 :${total20}원,
    30% 할인가 :${total30}원,
...
console.log(discountFunc(10000));
...
// 일반 반복 함수(생선된 위치 위/아래 어디든 호출가능)
func1(); //ok
function func1(){
    return console.log('일반함수테스트');
}
func1() //ok
...
const func2 = function(){
    return  console.log('익명함수테스트');
}
func2();
const func3 = ()=>{
    return  console.log('익명함수테스트 - 화살표버전');
}
func3();

// 2. 콜백함수 (함수 안의 또다른 함수 호출)
...
// HTML data-* 속성은 자바스크립트에서 dataset.* 읽을수있음
...
minusBtn.addEventListener('click', ()=>{
    ...
    let currentMNum = Number(countNumSpan.dataset.count);
    // 삼항 조건 연산자 활용 (수량이 1이상일때만 감소)
    currentMNum > 1 ? (()=>{
        console.log(typeof(currentMNum));
        let minusTotal = --currentMNum;
